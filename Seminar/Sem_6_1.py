#  # 2. Планеты вращаются вокруг звезд по эллиптическим орбитам. Назовем самой далекой планетой ту, 
...
# Каждая орбита представляет из себя кортеж из пары чисел - полуосей ее 
# эллипса. Площадь эллипса вычисляется по формуле S = piab, где a и b - длины полуосей эллипса. 
...
def find_farthest_orbit(list_of_orbits):
     # list_ell = [i for i in list_of_orbits if i[0] != i[1]]
     # list_areas = [(pi * i[0] * i[1]) for i in list_ell]
     # max_index = list_areas.index(max(list_areas))
     # return list_ell[max_index]
     return [i for i in list_of_orbits if i[0] * i[1] == max(i[0] * i[1] for i in list_of_orbits if i[0] != i[1])]
